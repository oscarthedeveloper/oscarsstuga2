...
exports.default = Kommandopalett;
exports.tolkaDatum = tolkaDatum;
...
const sok_1 = require("/sessions/rcw-01f8e9cwpsppjky27myncvez/mnt/oscarsstuga2/_to_delete/js/lib/sok");
...
const MINSTA_SOK = 2;
function Kommandopalett({ kommandon, onGaTill, onOppnaTraff, onFangad, onStang, }) {
    const butik = (0, Butik_1.useButik)();
    const [fraga, setFraga] = (0, react_1.useState)("");
    const [markerad, setMarkerad] = (0, react_1.useState)(0);
    const faltRef = (0, react_1.useRef)(null);
    const listRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        faltRef.current?.focus();
    }, []);
    const kalendernamn = (0, react_1.useMemo)(() => butik.kalendrar.map((k) => k.namn), [butik.kalendrar]);
    ...
    const fangst = (0, react_1.useMemo)(() => {
        const t = fangsttext.trim();
        if (t.length === 0)
            return null;
        return (0, tolka_1.tolkaFangst)(fangsttext, kalendernamn);
    }, [fangsttext, kalendernamn]);
    const traffar = (0, react_1.useMemo)(() => {
        if (tvingadFangst)
            return [];
        const q = fraga.trim();
        if (q.length < MINSTA_SOK)
            return [];
        return (0, sok_1.sok)(q, {
    ...
    const rader = (0, react_1.useMemo)(() => {
        const q = fraga.trim().toLowerCase();
        const ut = [];
        /* --- Fångst ---------------------------------------------------- */
        const kanFanga = !!fangst && fangst.titel.trim().length > 0;
        const fangstRad = kanFanga
            ? {
                nyckel: "fangst",
                grupp: "Fånga",
                etikett: fangst.sort === "handelse"
        ...
        const skriverNytt = tvingadFangst || (!!fangst && !fangst.tom) || traffar.length === 0;
        if (fangstRad && skriverNytt)
            ut.push(fangstRad);
        /* --- Datumhopp -------------------------------------------------- */
        ...
        if (datum) {
            ut.push({
                nyckel: "datum",
                grupp: "Gå till",
                etikett: `Gå till ${(0, tid_1.langtDatum)(datum)}`,
                hoger: "Datum",
                utfor: () => {
                    onGaTill(datum);
                    onStang();
        ...
        if (!tvingadFangst) {
            for (const k of kommandon) {
                if (q && !passar(k.namn.toLowerCase(), q))
                    continue;
                ut.push({
                    nyckel: k.id,
                    grupp: k.grupp,
    ...
    (0, react_1.useEffect)(() => setMarkerad(0), [fraga]);
    // Håll den markerade raden inom synhåll när man pilar sig neråt.
    (0, react_1.useEffect)(() => {
        const box = listRef.current;
        const rad = box?.querySelectorAll("[data-rad]")[markerad];
        rad?.scrollIntoView({ block: "nearest" });
    }, [markerad, rader.length]);
    return ((0, jsx_runtime_1.jsx)("div", { className: "palett-overlay", onClick: onStang, children: (0, jsx_runtime_1.jsxs)("div", { className: "palett", onClick: (e) => e.stopPropagation(), role: "dialog", "aria-label": "Kommandopalett", children: [(0, jsx_runtime_1.jsx)("input", { ref: faltRef, className: "palett-falt", placeholder: "Skriv f\u00F6r att f\u00E5nga, s\u00F6ka eller styra\u2026", value: fraga, onChange: (e) => setFraga(e.target.value), enterKeyHint: "done", autoComplete: "off", autoCorrect: "off", spellCheck: false, onKeyDown: (e) => {
                        if (e.key === "ArrowDown") {
                            e.preventDefault();
                    ...
                    } }), (0, jsx_runtime_1.jsxs)("div", { ref: listRef, className: "palett-lista tunnskroll", children: [rader.length === 0 && ((0, jsx_runtime_1.jsx)("p", { className: "palett-rad opacity-45", children: fraga.trim() ? "Inget matchar." : "Skriv något." })), rader.map((r, i) => {
                            const nyGrupp = i === 0 || rader[i - 1].grupp !== r.grupp;
                            return ((0, jsx_runtime_1.jsxs)("div", { children: [nyGrupp && (0, jsx_runtime_1.jsx)("p", { className: "palett-grupp", children: r.grupp }), (0, jsx_runtime_1.jsxs)("button", { type: "button", "data-rad": "", className: "palett-rad", "data-markerad": i === markerad ? "1" : "0", "data-fangst": r.slag === "fangst" ? "1" : "0", onMouseEnter: () => setMarkerad(i), onClick: r.utfor, children: [(0, jsx_runtime_1.jsxs)("span", { className: "min-w-0 flex-1", children: [(0, jsx_runtime_1.jsxs)("span", { className: "flex items-center gap-1.5 min-w-0", children: [r.slag && r.slag !== "fangst" && ((0, jsx_runtime_1.jsx)("span", { className: "palett-marke", "aria-hidden": "true", children: MARKE[r.slag] })), (0, jsx_runtime_1.jsx)("span", { className: "truncate", children: r.etikett })] }), r.under && ((0, jsx_runtime_1.jsx)("span", { className: "palett-under", children: r.under }))] }), (0, jsx_runtime_1.jsx)("span", { className: "pico opacity-55 shrink-0", children: r.hoger })] })] }, r.nyckel));
                        })] }), (0, jsx_runtime_1.jsxs)("div", { className: "border-t border-ink px-3 py-1.5 flex gap-3 overflow-x-auto chiprad", children: [(0, jsx_runtime_1.jsx)("span", { className: "pico opacity-45", children: "\u2191\u2193 Bl\u00E4ddra" }), (0, jsx_runtime_1.jsx)("span", { className: "pico opacity-45", children: "\u23CE V\u00E4lj" }), (0, jsx_runtime_1.jsx)("span", { className: "pico opacity-45", children: "+ Tvinga f\u00E5ngst" }), (0, jsx_runtime_1.jsx)("span", { className: "pico opacity-45 hidden md:inline", children: "Esc St\u00E4ng" })] })] }) }));
}
...
function passar(text, fraga) {
    if (text.includes(fraga))
        return true;
    let i = 0;
 ...
 */
function tolkaDatum(q, idag = new Date()) {
    ...
    if (s === "idag" || s === "nu")
        return d0;
    if (s === "imorgon" || s === "i morgon")
        return (0, tid_1.addDagar)(d0, 1);
    if (s === "igår" || s === "igar" || s === "i går")
        return (0, tid_1.addDagar)(d0, -1);
    if (s === "övermorgon" || s === "overmorgon")
        return (0, tid_1.addDagar)(d0, 2);
    const relativ = s.match(/^([+-])\s*(\d{1,3})$/);
    if (relativ) {
        const tecken = relativ[1] === "-" ? -1 : 1;
        return (0, tid_1.addDagar)(d0, tecken * Number(relativ[2]));
    }
    ...
    if (medManad) {
        ...
        const index = tid_1.MANADER.findIndex((m) => m.toLowerCase().startsWith(namn) ||
            tid_1.MANADER_KORT[tid_1.MANADER.indexOf(m)].toLowerCase() === namn.slice(0, 3));
        if (index === -1)
            return null;
