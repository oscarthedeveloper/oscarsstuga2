...
exports.STANDARDKALENDRAR = exports.TomtLager = exports.LokaltLager = exports.ANTAL_TONER = void 0;
exports.klamTon = klamTon;
exports.nu = nu;
exports.stadaGravstenar = stadaGravstenar;
exports.normalisera = normalisera;
exports.normaliseraKalender = normaliseraKalender;
exports.normaliseraUppgift = normaliseraUppgift;
exports.normaliseraAnteckning = normaliseraAnteckning;
exports.normaliseraSida = normaliseraSida;
exports.rord = rord;
exports.gravsatt = gravsatt;
exports.nyId = nyId;
exports.levande = levande;
exports.laggTillKalender = laggTillKalender;
exports.andraKalender = andraKalender;
exports.taBortKalender = taBortKalender;
exports.sorteraUppgifter = sorteraUppgifter;
exports.vaxlaKlar = vaxlaKlar;
exports.sorteraAnteckningar = sorteraAnteckningar;
const upprepning_1 = require("./upprepning");
 ...
 */
exports.ANTAL_TONER = 6;
function klamTon(ton) {
    return ((Math.round(ton) % exports.ANTAL_TONER) + exports.ANTAL_TONER) % exports.ANTAL_TONER;
}
...
exports.LokaltLager = LokaltLager;
 ...
 */
function stadaGravstenar(o, idag = new Date()) {
    const grans = new Date(idag.getTime() - GRAVSTEN_DYGN * 86400000).toISOString();
    const lever = (x) => !x.raderad || x.raderad > grans || !x.synkad;
    return {
...
function normalisera(h) {
    return {
        id: h.id ?? nyId(),
        titel: h.titel ?? "Utan titel",
        anteckning: h.anteckning ?? "",
        plats: h.plats ?? "",
        start: h.start ?? (0, tid_1.stampel)(new Date()),
        slut: h.slut ?? (0, tid_1.stampel)(new Date()),
        heldag: !!h.heldag,
        kalenderId: h.kalenderId ?? "arbete",
        upprepning: h.upprepning
            ? { ...upprepning_1.STANDARD_UPPREPNING, ...h.upprepning }
            : null,
        undantag: h.undantag ?? [],
...
function normaliseraKalender(k) {
    return {
        id: k.id ?? nyId(),
        namn: k.namn ?? "Namnlös",
        ton: klamTon(k.ton ?? 0),
...
function normaliseraAnteckning(a) {
    return {
        id: a.id ?? nyId(),
        titel: a.titel ?? "",
        brodtext: a.brodtext ?? "",
...
function normaliseraSida(x) {
    ...
    const rimlig = x.data && typeof x.data === "object" && !Array.isArray(x.data)
        ? x.data
        : {};
    return {
        id: x.id ?? nyId(),
...
function levande(lista) {
    return lista.filter((x) => !x.raderad);
}
exports.STANDARDKALENDRAR = [
...
/* ==================================================================
   ...
   ================================================================== */
function laggTillKalender(o, namn, ton, id = nyId()) {
...
function andraKalender(o, id, delar) {
    return {
        ...o,
        kalendrar: o.kalendrar.map((k) => k.id === id
            ? rord({
                ...k,
                ...delar,
 ...
 */
function taBortKalender(o, id, flyttaTill) {
    const levandeKal = levande(o.kalendrar);
    if (levandeKal.length <= 1)
        return o;
    ...
    const flyttmal = flyttaTill && flyttaTill !== id && kvar.some((k) => k.id === flyttaTill)
        ? flyttaTill
        : null;
    ...
    return {
        kalendrar: o.kalendrar.map((k) => k.id === id ? gravsatt(k, tidpunkt) : k),
 ...
 */
function sorteraUppgifter(lista) {
...
function vaxlaKlar(u, tidpunkt = nu()) {
    return rord({ ...u, klar: !u.klar, klarVid: u.klar ? null : tidpunkt }, tidpunkt);
}
/* ==================================================================
   ANTECKNINGAR
   ================================================================== */
 ...
 */
function sorteraAnteckningar(lista) {
    return [...lista].sort((a, b) => {
        if (a.nalad !== b.nalad)
            return a.nalad ? -1 : 1;
        if (a.andrad !== b.andrad)
            return b.andrad.localeCompare(a.andrad);
