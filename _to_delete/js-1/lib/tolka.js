...
exports.tolkaFangst = tolkaFangst;
...
const ATEN = " ";
function tolkaFangst(text, kalendernamn = [], nuTid = new Date()) {
    const kvar = Array.from(text);
    const delar = [];
    const idag = (0, tid_1.startAvDag)(nuTid);
    ...
    /* --- 1. Uttalad sort ------------------------------------------- */
    let tvingadSort = null;
    const sortM = ta(/(?:^|\s)(uppgift|todo|att\s*göra|händelse|handelse|möte|mote)\s*:\s*/i);
    if (sortM) {
    ...
    let startMin = null;
    let slutMin = null;
    const bådaKlockslag = (m) => klockslag(m[1], m[2]) !== null && klockslag(m[3], m[4]) !== null;
    const spannMedKl = ta(/\bkl\.?\s*(\d{1,2})(?:[:.](\d{2}))?\s*(?:-|–|—|till)\s*(\d{1,2})(?:[:.](\d{2}))?(?=\s|$)/i, bådaKlockslag);
    const spann = spannMedKl ??
        ta(/\b(\d{1,2})[:.](\d{2})\s*(?:-|–|—|till)\s*(\d{1,2})(?:[:.](\d{2}))?(?=\s|$)/, bådaKlockslag);
    if (spann) {
        ...
        const ettKlockslag = (m) => klockslag(m[1], m[2]) !== null;
        const enkel = ta(/\bkl\.?\s*(\d{1,2})(?:[:.](\d{2}))?(?=\s|$)/i, ettKlockslag) ??
    ...
    if (halv) {
        ...
        const tim = ta(/\b(?:i\s+)?(\d{1,3})(?:[.,](\d))?\s*(?:h|tim|timme|timmar)\b/i);
        const min = ta(/\b(?:i\s+)?(\d{1,3})\s*(?:min|minut|minuter)\b/i);
        let summa = 0;
        if (tim)
            summa += Number(tim[1]) * 60 + (tim[2] ? Number(tim[2]) * 6 : 0);
        if (min)
            summa += Number(min[1]);
        if (summa > 0) {
            langdMin = summa;
            const rat = [tim?.[0].trim(), min?.[0].trim()].filter(Boolean).join(" ");
            delar.push({ slag: "langd", text: rat, tolkning: (0, tid_1.minuterTillText)(summa) });
        }
    }
    /* --- 7. Datum --------------------------------------------------- */
    const datum = tolkaDatumdel(ta, idag);
    if (datum)
        delar.push(datum.del);
    /* --- 8. Titeln är det som blev över ----------------------------- */
    const titel = stada(arbets());
    /* --- 9. Sätt ihop ----------------------------------------------- */
    const harTid = startMin !== null;
    const sort = tvingadSort ?? (harTid || heldag ? "handelse" : "uppgift");
    const fangst = {
        sort,
        titel,
        start: null,
    ...
    if (sort === "uppgift") {
        fangst.forfaller = datum ? (0, tid_1.nyckel)(datum.dag) : null;
        return fangst;
    }
    /* Händelse. */
    if (heldag) {
        ...
        fangst.heldag = true;
        fangst.start = (0, tid_1.stampel)(dag);
        fangst.slut = (0, tid_1.stampel)((0, tid_1.addDagar)(dag, 1));
        return fangst;
    }
    /*
    ...
    if (!datum && startMin !== null) {
        ...
        if (passerat) {
            dag = (0, tid_1.addDagar)(idag, 1);
            delar.push({
                slag: "datum",
                text: "—",
                tolkning: `${(0, tid_1.kortDatum)(dag)} (tiden har passerat idag)`,
            });
        }
    }
    const s = startMin ?? 9 * 60;
    const e = slutMin ?? s + (langdMin ?? STANDARDLANGD_MIN);
    fangst.start = (0, tid_1.stampel)((0, tid_1.medMinuter)(dag, s));
    fangst.slut = (0, tid_1.stampel)((0, tid_1.medMinuter)(dag, e));
    return fangst;
}
/* ==================================================================
   DATUM
 ...
 */
function tolkaDatumdel(ta, idag) {
    const svar = (dag, text) => ({
        dag,
        del: { slag: "datum", text: text.trim(), tolkning: (0, tid_1.kortDatum)(dag) },
    });
    /* ISO: 2026-12-24 */
    const iso = ta(/\b(\d{4})-(\d{1,2})-(\d{1,2})\b/, (m) => giltigDel(Number(m[1]), Number(m[2]), Number(m[3])));
    if (iso) {
    ...
    const snett = ta(/\b(\d{1,2})\/(\d{1,2})(?:\s+(\d{4}))?\b/, (m) => giltigDel(m[3] ? Number(m[3]) : idag.getFullYear(), Number(m[2]), Number(m[1])));
    if (snett) {
        ...
        if (giltigt(d)) {
            ...
            const rullad = !snett[3] && d.getTime() < idag.getTime()
    ...
    const medManad = ta(/\b(?:den\s+)?(\d{1,2})(?::e|:a)?\s+([\p{L}]{3,})\.?(?:\s+(\d{4}))?\b/u, 
    ...
    if (medManad) {
        ...
        if (index !== -1) {
            ...
            if (giltigt(d)) {
                const rullad = !medManad[3] && d.getTime() < idag.getTime()
    ...
    if (idagM)
        return svar(idag, idagM[0]);
    const imorgonM = ta(/(?:^|\s)(?:imorgon|i\s+morgon|imorron)(?=\s|$)/i);
    if (imorgonM)
        return svar((0, tid_1.addDagar)(idag, 1), imorgonM[0]);
    const overM = ta(/(?:^|\s)(?:övermorgon|overmorgon)(?=\s|$)/i);
    if (overM)
        return svar((0, tid_1.addDagar)(idag, 2), overM[0]);
    const igarM = ta(/(?:^|\s)(?:igår|igar|i\s+går)(?=\s|$)/i);
    if (igarM)
        return svar((0, tid_1.addDagar)(idag, -1), igarM[0]);
    /* "om 3 dagar", "om en vecka", "om 2 månader" */
    const om = ta(/(?:^|\s)om\s+(\d{1,3}|en|ett|två|tre)\s+(dag|dagar|vecka|veckor|månad|månader|manad|manader)(?=\s|$)/i);
    if (om) {
        const antal = raknerord(om[1]);
        const enhet = om[2].toLowerCase();
        if (enhet.startsWith("dag"))
            return svar((0, tid_1.addDagar)(idag, antal), om[0]);
        if (enhet.startsWith("veck"))
            return svar((0, tid_1.addDagar)(idag, antal * 7), om[0]);
        return svar((0, tid_1.addManader)(idag, antal), om[0]);
    }
    /* "nästa vecka" */
    const nastaVecka = ta(/(?:^|\s)nästa\s+vecka(?=\s|$)/i);
    if (nastaVecka)
        return svar((0, tid_1.addDagar)(idag, 7), nastaVecka[0]);
    /*
     ...
     * är tvetydigt.
     */
    const veckodag = ta(/(?:^|\s)(?:(nästa|nasta)\s+)?(?:på\s+|i\s+)?(söndag|sondag|måndag|mandag|tisdag|onsdag|torsdag|fredag|lördag|lordag|sön|mån|man|tis|ons|tor|fre|lör|lor)(?=\s|$)/i, (m) => veckodagsIndex(m[2]) !== -1);
    if (veckodag) {
        ...
        if (index !== -1) {
            ...
            if (veckodag[1])
                steg += 7;
            return svar((0, tid_1.addDagar)(idag, steg), veckodag[0]);
        }
    }
    return null;
}
/* ==================================================================
   SMÅDELAR
   ================================================================== */
function klockslag(timme, minut) {
    const t = Number(timme);
    const m = minut ? Number(minut) : 0;
    if (!Number.isFinite(t) || t > 23 || m > 59)
        return null;
    return t * 60 + m;
}
function minuterTillKlocka(minuter) {
    const m = ((minuter % 1440) + 1440) % 1440;
    return (0, tid_1.klocka)((0, tid_1.medMinuter)(new Date(2000, 0, 1), m));
}
function manadsIndex(ord) {
    const o = ord.toLowerCase().replace(/\.$/, "");
    for (let i = 0; i < tid_1.MANADER.length; i++) {
        if (tid_1.MANADER[i].toLowerCase() === o)
            return i;
        if (tid_1.MANADER_KORT[i].toLowerCase() === o)
            return i;
    }
    // Prefix sist: "mar" skall inte fastna på "maj" före "mars".
    for (let i = 0; i < tid_1.MANADER.length; i++) {
        if (o.length >= 3 && tid_1.MANADER[i].toLowerCase().startsWith(o))
            return i;
    }
    return -1;
}
function veckodagsIndex(ord) {
    const o = utanDiakriter(ord.toLowerCase());
    for (let i = 0; i < tid_1.VECKODAGAR.length; i++) {
        if (utanDiakriter(tid_1.VECKODAGAR[i].toLowerCase()) === o)
            return i;
        if (utanDiakriter(tid_1.VECKODAGAR_KORT[i].toLowerCase()) === o)
            return i;
    }
    return -1;
}
/** "lördag" och "lordag" skall båda gå fram. */
function utanDiakriter(s) {
    return s.replace(/[åä]/g, "a").replace(/ö/g, "o");
}
function raknerord(s) {
    const ord = { en: 1, ett: 1, två: 2, tre: 3 };
...
function giltigDel(ar, manad, dag) {
    if (manad < 1 || manad > 12 || dag < 1 || dag > 31)
        return false;
