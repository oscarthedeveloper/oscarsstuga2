...
const ordning = (lista) => (0, butik_1.sorteraUppgifter)(lista).map((x) => x.id);
...
prov("ett satt datum går före inget datum alls", () => {
    ...
    lika(ordning([u("utan", 2), u("med", 2, { forfaller: "2026-12-24" })]), ["med", "utan"]);
});
prov("styrkan väger tyngre än datumet", () => {
    ...
    lika(ordning([u("svag", 3, { forfaller: "2026-08-13" }), u("stark", 1)]), ["stark", "svag"]);
});
...
prov("sorteringen muterar inte sitt indata", () => {
    ...
    (0, butik_1.sorteraUppgifter)(lista);
    lika(lista, kopia);
});
/* --- normalisering ------------------------------------------------ */
prov("styrka utanför skalan hamnar i mitten", () => {
    lika((0, butik_1.normaliseraUppgift)({ prioritet: 0 }).prioritet, 2);
    lika((0, butik_1.normaliseraUppgift)({ prioritet: 9 }).prioritet, 2);
    lika((0, butik_1.normaliseraUppgift)({}).prioritet, 2);
    lika((0, butik_1.normaliseraUppgift)({ prioritet: 1 }).prioritet, 1);
    lika((0, butik_1.normaliseraUppgift)({ prioritet: 3 }).prioritet, 3);
});
prov("tomt förfallodatum blir null, inte tom sträng", () => {
    ...
    lika((0, butik_1.normaliseraUppgift)({ forfaller: "" }).forfaller, null);
});
/* --- avbockning --------------------------------------------------- */
prov("avbockning sätter tidpunkt och gör posten osynkad", () => {
    const start = { ...u("a", 1), synkad: true };
    const efter = (0, butik_1.vaxlaKlar)(start, "2026-08-12T12:00:00.000Z");
...
prov("att ångra avbockningen tar bort tidpunkten", () => {
    const klar = (0, butik_1.vaxlaKlar)(u("a", 1), "2026-08-12T12:00:00.000Z");
    const angrad = (0, butik_1.vaxlaKlar)(klar, "2026-08-12T13:00:00.000Z");
...
function bas() {
    return {
        ...
        sidor: [],
        kalendrar: [
            (0, butik_1.normaliseraKalender)({ id: "a", namn: "Arbete", ton: 3 }),
            (0, butik_1.normaliseraKalender)({ id: "s", namn: "Studier", ton: 0 }),
        ],
        handelser: [
...
prov("uppgifter följer med när deras kalender flyttas", () => {
    ...
    lika((0, butik_1.levande)(o.uppgifter).map((x) => [x.id, x.kalenderId]), [
        ["u1", "a"],
        ["u2", "a"],
    ]);
...
prov("uppgifter gravsätts när kalendern raderas utan flyttmål", () => {
    ...
    lika((0, butik_1.levande)(o.uppgifter).map((x) => x.id), ["u2"]);
...
prov("ingen uppgift blir kvar med en kalender som inte finns", () => {
    for (const mal of ["a", null]) {
        ...
        for (const x of (0, butik_1.levande)(o.uppgifter)) {
...
prov("osynkade uppgifter räknas med i kön", () => {
    const o = bas();
    lika((0, synk_1.osynkade)(o.uppgifter).length, 2);
    // Två uppgifter, en händelse och två kalendrar, alla nyskapade.
    lika((0, synk_1.antalIvag)(o), 5);
});
