...
exports.nyckelFor = nyckelFor;
exports.hittaLankar = hittaLankar;
exports.byggRegister = byggRegister;
exports.slaUpp = slaUpp;
exports.delaText = delaText;
exports.bakatlankar = bakatlankar;
exports.svavandeLankar = svavandeLankar;
const LANK = /\[\[([^\]\n]+)\]\]/g;
/**
 ...
 */
function byggRegister(kalla) {
    const reg = new Map();
    const lagg = (slag, id, titel) => {
        const n = nyckelFor(titel);
        if (!n || reg.has(n))
            return;
    ...
    for (const a of kalla.anteckningar ?? [])
        lagg("anteckning", a.id, a.titel);
    for (const h of kalla.handelser)
        lagg("handelse", h.id, h.titel);
    for (const u of kalla.uppgifter)
        lagg("uppgift", u.id, u.titel);
    return reg;
}
function slaUpp(reg, titel) {
...
function delaText(text, reg) {
    ...
    for (const m of text.matchAll(LANK)) {
        const i = m.index ?? 0;
        if (i > sist)
            ut.push({ typ: "text", text: text.slice(sist, i) });
        const titel = m[1].trim();
        ut.push({ typ: "lank", text: titel, mal: slaUpp(reg, titel) });
        sist = i + m[0].length;
    }
    if (sist < text.length)
        ut.push({ typ: "text", text: text.slice(sist) });
    return ut;
}
/**
 ...
 */
function bakatlankar(kalla, mal) {
    const sokt = nyckelFor(mal.titel);
    if (!sokt)
        return [];
    const ut = [];
    const provaKalla = (slag, id, titel, kropp) => {
 ...
 */
function radMed(kropp, sokt, bredd = 90) {
    const rader = kropp.split(/\n+/);
    const traff = rader.find((r) => hittaLankar(r).some((t) => nyckelFor(t) === sokt)) ??
        rader[0] ??
        "";
