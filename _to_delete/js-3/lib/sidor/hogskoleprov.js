...
exports.TOM_HP = exports.HOGSTA_NORMERAT = exports.PROVDELAR = void 0;
exports.tal = tal;
exports.tolkaTermin = tolkaTermin;
exports.terminText = terminText;
exports.terminOrdning = terminOrdning;
exports.tolkaHpData = tolkaHpData;
exports.sorteradeResultat = sorteradeResultat;
exports.senasteResultat = senasteResultat;
exports.bastaResultat = bastaResultat;
exports.avstand = avstand;
exports.avrunda = avrunda;
exports.poangtext = poangtext;
exports.skillnadstext = skillnadstext;
exports.delresultat = delresultat;
exports.svagasteDelen = svagasteDelen;
exports.gruppsumma = gruppsumma;
exports.dygnKvar = dygnKvar;
exports.nedrakningstext = nedrakningstext;
exports.sorteradeDatum = sorteradeDatum;
...
function tolkaTermin(rå) {
    ...
    if (!m)
        return null;
    const sasong = m[1].startsWith("v") ? "var" : "host";
...
function terminOrdning(t) {
    if (!t)
        return Number.POSITIVE_INFINITY;
    return t.ar * 2 + (t.sasong === "host" ? 1 : 0);
}
function tolkaTerminVarde(rå) {
    if (typeof rå === "string")
        return tolkaTermin(rå);
    if (!arObjekt(rå))
        return null;
    const sasong = text(rå.sasong);
    const ar = tal(rå.ar, 1900, 2200);
    if ((sasong !== "var" && sasong !== "host") || ar === null)
        return null;
...
const datumtext = (x) => /^\d{4}-\d{2}-\d{2}$/.test(text(x)) ? text(x) : "";
...
function tolkaHpData(rå) {
    if (!arObjekt(rå))
        return exports.TOM_HP;
    return {
        mal: tal(rå.mal, 0, exports.HOGSTA_NORMERAT),
        resultat: lista(rå.resultat, (rad, i) => ({
            ...
            normerat: tal(rad.normerat, 0, exports.HOGSTA_NORMERAT),
        ...
        larosaten: lista(rå.larosaten, (rad, i) => ({
            ...
            poang: tal(rad.poang, 0, exports.HOGSTA_NORMERAT),
        })),
    };
}
function tolkaDelar(rå) {
    if (!arObjekt(rå))
        return {};
    const ut = {};
    for (const d of exports.PROVDELAR) {
...
function sorteradeResultat(data) {
    return [...data.resultat].sort((a, b) => terminOrdning(a.termin) - terminOrdning(b.termin) ||
...
function bastaResultat(data) {
    let bast = null;
    for (const r of data.resultat) {
        if (r.normerat === null)
            continue;
        if (!bast || r.normerat > bast.normerat)
            bast = r;
    }
    return bast;
}
 ...
 */
function avstand(data) {
    const bast = bastaResultat(data)?.normerat ?? null;
    const rad = (id, etikett, krav) => ({
        id,
        etikett,
        krav,
    ...
    const ut = data.larosaten.map((l) => rad(l.id, [l.namn, l.termin].filter(Boolean).join(" · ") || "Namnlöst", l.poang));
    if (data.mal !== null)
        ut.push(rad("mal", "Eget mål", data.mal));
    return ut;
}
...
function dygnKvar(datumnyckel, idag = new Date()) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(datumnyckel))
        return null;
    const mal = (0, tid_1.startAvDag)((0, tid_1.tolka)(datumnyckel));
    const nu = (0, tid_1.startAvDag)(idag);
    return Math.round((mal.getTime() - nu.getTime()) / 86400000);
}
function nedrakningstext(dygn) {
    if (dygn === null)
        return "";
    if (dygn === 0)
        return "Idag";
...
function sorteradeDatum(data, idag = new Date()) {
    const idagsnyckel = (0, tid_1.nyckel)((0, tid_1.startAvDag)(idag));
