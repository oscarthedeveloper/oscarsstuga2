...
exports.delaMarkering = delaMarkering;
exports.renText = renText;
exports.vaxlaMarkering = vaxlaMarkering;
exports.teckenForTangent = teckenForTangent;
/*
...
function delaMarkering(text) {
    ...
    for (const m of text.matchAll(MONSTER)) {
        const i = m.index ?? 0;
        if (i > sist)
            ut.push({ slag: "text", text: text.slice(sist, i) });
        if (m[1] !== undefined)
            ut.push({ slag: "fet", text: m[1] });
        else if (m[2] !== undefined)
            ut.push({ slag: "kursiv", text: m[2] });
        else
            ut.push({ slag: "kod", text: m[3] });
        sist = i + m[0].length;
    }
    if (sist < text.length)
        ut.push({ slag: "text", text: text.slice(sist) });
    return ut;
}
/** Texten utan markeringstecken. För utdrag och sökning. */
function renText(text) {
 ...
 */
function vaxlaMarkering(text, start, slut, tecken) {
    const c = tecken[0];
    ...
    while (a < b && text[a] === c)
        a += 1;
    while (b > a && text[b - 1] === c)
        b -= 1;
    const karna = text.slice(a, b);
    // Löpan av markörtecken utanför kärnan, på var sida.
    let fore = 0;
    while (a - fore - 1 >= 0 && text[a - fore - 1] === c)
        fore += 1;
    let efter = 0;
    while (b + efter < text.length && text[b + efter] === c)
        efter += 1;
...
function teckenForTangent(tangent) {
