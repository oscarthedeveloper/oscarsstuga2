...
const kopplingar_1 = require("../lib/kopplingar");
...
const h = (id, titel, anteckning = "") => (0, butik_1.normalisera)({
    id,
    titel,
    anteckning,
    kalenderId: "arbete",
...
const u = (id, titel, anteckning = "") => (0, butik_1.normaliseraUppgift)({ id, titel, anteckning, kalenderId: "arbete" });
const a = (id, titel, brodtext = "") => (0, butik_1.normaliseraAnteckning)({ id, titel, brodtext, kalenderId: "arbete" });
process.stdout.write("\nKOPPLINGAR\n");
prov("hittar länkar i text", () => {
    lika((0, kopplingar_1.hittaLankar)("se [[Budget]] och [[Möte med Anna]]"), [
        "Budget",
        "Möte med Anna",
    ]);
});
prov("dubbletter räknas en gång", () => {
    lika((0, kopplingar_1.hittaLankar)("[[Budget]] igen [[budget]]"), ["Budget"]);
});
prov("tomma och trasiga länkar hoppas över", () => {
    lika((0, kopplingar_1.hittaLankar)("[[]] [[ ]] [[ok]]"), ["ok"]);
    lika((0, kopplingar_1.hittaLankar)("bara [[ en halv"), []);
    lika((0, kopplingar_1.hittaLankar)(""), []);
});
prov("skiftläge och blanksteg avgör inte", () => {
    lika((0, kopplingar_1.nyckelFor)("  Kvartals   Rapporten "), "kvartals rapporten");
});
prov("registret slår upp över alla tre sorterna", () => {
    ...
    const reg = (0, kopplingar_1.byggRegister)(kalla);
    lika((0, kopplingar_1.slaUpp)(reg, "kickoff")?.slag, "handelse");
    lika((0, kopplingar_1.slaUpp)(reg, "BOKA LOKAL")?.slag, "uppgift");
    lika((0, kopplingar_1.slaUpp)(reg, "Planering")?.slag, "anteckning");
    lika((0, kopplingar_1.slaUpp)(reg, "finns inte"), null);
});
prov("anteckningen vinner när titlar krockar", () => {
    const reg = (0, kopplingar_1.byggRegister)({
        handelser: [h("h1", "Budget")],
        uppgifter: [],
        anteckningar: [a("a1", "Budget")],
    });
    lika((0, kopplingar_1.slaUpp)(reg, "budget")?.id, "a1");
});
prov("texten delas i bitar utan att tappa tecken", () => {
    ...
    const bitar = (0, kopplingar_1.delaText)("före [[Budget]] efter", reg);
    lika(bitar.length, 3);
    lika(bitar.map((b) => (b.typ === "text" ? b.text : b.text)).join(""), "före Budget efter");
    lika(bitar[1].typ, "lank");
});
prov("länk utan mål är inte ett fel", () => {
    const bitar = (0, kopplingar_1.delaText)("se [[Finns inte]]", (0, kopplingar_1.byggRegister)({ handelser: [], uppgifter: [] }));
...
prov("bakåtlänkar hittar allt som pekar hit", () => {
    ...
    const in_ = (0, kopplingar_1.bakatlankar)(kalla, { titel: "Planering", id: "a1" });
    lika(in_.length, 2);
...
prov("en post länkar inte till sig själv", () => {
    ...
    lika((0, kopplingar_1.bakatlankar)(kalla, { titel: "Loop", id: "a1" }).length, 0);
});
prov("bakåtlänken bär meningen den stod i", () => {
    ...
    const in_ = (0, kopplingar_1.bakatlankar)(kalla, { titel: "Rapporten", id: "a2" });
    lika(in_.length, 1);
...
prov("svävande länkar listas för att kunna skapas", () => {
    ...
    lika((0, kopplingar_1.svavandeLankar)(kalla), ["Agendan"], "Planering finns ju");
});
