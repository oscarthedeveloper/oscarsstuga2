...
globalThis.localStorage = {
    getItem: (k) => minne.get(k) ?? null,
    setItem: (k, v) => void minne.set(k, v),
    removeItem: (k) => void minne.delete(k),
};
 ...
 */
function falskKlientPerTabell(hamta, skickat) {
    return {
        from(tabell) {
            const byggare = {};
...
function h(id, titel, andrad, extra = {}) {
...
function k(id, namn, andrad, extra = {}) {
    return (0, butik_1.normaliseraKalender)({ id, namn, ton: 0, andrad, ...extra });
}
process.stdout.write("\nSynkmotorn\n");
/* --- grundfall ---------------------------------------------------- */
prov("post som bara finns lokalt behålls", () => {
    const ut = (0, synk_1.sammanfoga)([h("a", "Möte", T("0T10:00:00Z"))], []);
    lika(ut.map((x) => x.titel), ["Möte"]);
});
prov("post som bara finns i molnet hämtas hem", () => {
    const ut = (0, synk_1.sammanfoga)([], [h("a", "Möte", T("0T10:00:00Z"), { synkad: true })]);
    lika(ut.map((x) => x.titel), ["Möte"]);
    lika(ut[0].synkad, true);
});
prov("nyare lokal ändring vinner över molnet", () => {
    const ut = (0, synk_1.sammanfoga)([h("a", "Telefonens titel", T("0T12:00:00Z"))], [h("a", "Molnets titel", T("0T11:00:00Z"), { synkad: true })]);
...
prov("nyare fjärrändring vinner över den lokala", () => {
    const ut = (0, synk_1.sammanfoga)([h("a", "Telefonens titel", T("0T11:00:00Z"))], [h("a", "Molnets titel", T("0T12:00:00Z"), { synkad: true })]);
...
prov("exakt lika stämplar avgörs likadant på varje enhet", () => {
    ...
    // i all evighet.
    const a = (0, synk_1.sammanfoga)([h("a", "Enhet A", T("0T12:00:00Z"))], [h("a", "Molnet", T("0T12:00:00Z"), { synkad: true })]);
    const b = (0, synk_1.sammanfoga)([h("a", "Enhet B", T("0T12:00:00Z"))], [h("a", "Molnet", T("0T12:00:00Z"), { synkad: true })]);
    lika(a[0].titel, "Molnet");
    lika(b[0].titel, "Molnet");
});
/* --- gravstenar --------------------------------------------------- */
prov("borttagning i molnet slår igenom lokalt", () => {
    const ut = (0, synk_1.sammanfoga)([h("a", "Möte", T("0T10:00:00Z"), { synkad: true })], [h("a", "Möte", T("0T11:00:00Z"), { synkad: true, raderad: T("0T11:00:00Z") })]);
    lika(ut.length, 1, "gravstenen skall ligga kvar i lagret");
    lika((0, butik_1.levande)(ut).length, 0, "men posten skall inte synas");
});
prov("en post som ändrats lokalt EFTER borttagningen återuppstår", () => {
    ...
    const ut = (0, synk_1.sammanfoga)([h("a", "Omskriven", T("0T12:00:00Z"))], [h("a", "Möte", T("0T11:00:00Z"), { synkad: true, raderad: T("0T11:00:00Z") })]);
    lika((0, butik_1.levande)(ut).length, 1);
...
prov("en lokalt raderad post stannar raderad efter synk", () => {
    ...
    const lokal = (0, butik_1.gravsatt)(h("a", "Möte", T("0T10:00:00Z")), T("2T09:00:00Z"));
    const ut = (0, synk_1.sammanfoga)([lokal], [h("a", "Möte", T("0T10:00:00Z"), { synkad: true })]);
    lika((0, butik_1.levande)(ut).length, 0);
});
prov("gravstenar städas först när de nått molnet", () => {
    const gammal = (0, butik_1.gravsatt)(h("a", "Gammal", T("0T10:00:00Z")), "2020-01-01T00:00:00Z");
    const o = {
    ...
    const kvar = (0, butik_1.stadaGravstenar)(o, new Date("2026-08-12T00:00:00Z"));
...
prov("färska gravstenar rörs inte", () => {
    const fersk = {
        ...(0, butik_1.gravsatt)(h("a", "Ny", T("0T10:00:00Z")), "2026-08-11T00:00:00Z"),
        synkad: true,
    };
    const kvar = (0, butik_1.stadaGravstenar)({ handelser: [fersk], kalendrar: [], uppgifter: [],
        anteckningar: [],
        sidor: [] }, new Date("2026-08-12T00:00:00Z"));
...
prov("kalenderns synlighet är en inställning per enhet", () => {
    ...
    const ut = (0, synk_1.sammanfogaKalendrar)([k("arbete", "Arbete", T("0T10:00:00Z"), { synlig: false })], [k("arbete", "Jobb", T("0T12:00:00Z"), { synlig: true, synkad: true })]);
...
prov("borttagen kalender tar med sig sina händelser som gravstenar", () => {
    ...
    const ut = (0, butik_1.taBortKalender)(o, "s", null);
    lika((0, butik_1.levande)(ut.kalendrar).map((x) => x.id), ["a"]);
    lika((0, butik_1.levande)(ut.handelser).length, 0);
...
prov("flyttade händelser stämplas om så de skickas upp", () => {
    ...
    const ut = (0, butik_1.taBortKalender)(o, "s", "a");
...
prov("osynkade poster räknas rätt", () => {
    ...
    lika((0, synk_1.osynkade)(o.handelser).map((x) => x.id), ["b"]);
    lika((0, synk_1.antalIvag)(o), 2, "kalendern är också osynkad");
});
prov("en ändring markerar posten som osynkad", () => {
    const start = { ...h("a", "Möte", T("0T10:00:00Z")), synkad: true };
    const efter = (0, butik_1.rord)({ ...start, titel: "Nytt namn" }, T("2T10:00:00Z"));
...
prov("en offlinekö överlever flera varv utan nät", () => {
    ...
    let o = { handelser: [], kalendrar: [], uppgifter: [],
        anteckningar: [],
        sidor: [] };
    ...
    lika((0, synk_1.antalIvag)(o), 3);
    // Molnet svarar med en fjärde post som ingen av enheterna sett.
    const efter = (0, synk_1.sammanfoga)(o.handelser, [
        h("d", "Fyra", T("0T09:00:00Z"), { synkad: true }),
    ]);
    lika(efter.length, 4);
    lika((0, synk_1.osynkade)(efter).map((x) => x.id), ["a", "b", "c"]);
});
/* --- markören ----------------------------------------------------- */
prov("markören backas med marginal innan den används", () => {
    ...
    lika((0, synk_1.backaMarkor)(m, 60000), "2026-08-12T09:59:00.000Z");
});
prov("nolltiden backas inte förbi epoken", () => {
    lika((0, synk_1.backaMarkor)("1970-01-01T00:00:00.000Z"), "1970-01-01T00:00:00.000Z");
    lika((0, synk_1.backaMarkor)("trasig"), "1970-01-01T00:00:00.000Z");
});
provAsync("en synkrunda flyttar markören ENDAST med hämtade rader", async () => {
    ...
    await (0, synk_1.synka)(lokal, anvandare, klient);
    const markorEfter = (0, synk_1.lasMarkor)(anvandare);
...
provAsync("markören flyttas fram av hämtade rader", async () => {
    ...
    const resultat = await (0, synk_1.synka)({ handelser: [], kalendrar: [], uppgifter: [],
        anteckningar: [],
        sidor: [] }, anvandare, klient);
    lika((0, synk_1.lasMarkor)(anvandare), "2026-08-12T10:00:03.000Z");
    lika((0, butik_1.levande)(resultat.data.handelser).map((x) => x.titel), ["Från molnet"]);
});
prov("oförändrad hämtning ger tillbaka samma lista", () => {
    ...
    const ut = (0, synk_1.sammanfoga)(lokal, fran_molnet);
...
prov("en verklig ändring ger däremot en ny lista", () => {
    ...
    const ut = (0, synk_1.sammanfoga)(lokal, nyare);
    lika(ut === lokal, false);
...
prov("sammanfogningen muterar inte sitt indata", () => {
    ...
    (0, synk_1.sammanfoga)(lokala, [h("a", "Annat", T("0T12:00:00Z"), { synkad: true })]);
    lika(lokala, kopia);
});
async function kor() {
...
provAsync("en sida skickas upp till sidor-tabellen", async () => {
    ...
    const resultat = await (0, synk_1.synka)(lokal, anvandare, klient);
...
provAsync("en sida från molnet sammanfogas in", async () => {
    ...
    (0, synk_1.nollstallMarkor)(anvandare);
    const skickat = {};
    const klient = falskKlientPerTabell((tabell) => tabell === "sidor"
        ? [
            {
                agare: anvandare,
                id: "fornsvenska",
    ...
    lika((0, synk_1.lasMarkor)(anvandare), "2026-08-12T11:00:01.000Z");
});
