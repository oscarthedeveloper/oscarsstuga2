...
const DATA: VinData = tolkaVinData({
  nastaKod: 5,
  viner: [
    {
      id: "a",
...
prov("tal tolkas med mellanrum, komma, kr och procent", () => {
  lika(tolkaTal("1299"), 1299);
  lika(tolkaTal("1 299"), 1299, "vanligt mellanrum");
  lika(tolkaTal("1 299"), 1299, "hårt mellanrum, som utskriften");
  lika(tolkaTal("89 kr"), 89);
  lika(tolkaTal("13,5"), 13.5);
  lika(tolkaTal("13,5 %"), 13.5);
  lika(tolkaTal(3.7), 3.7);
});
...
prov("tomt är okänt, inte noll", () => {
  ...
  lika(tolkaTal(""), null);
  lika(tolkaTal("   "), null);
  lika(tolkaTal("abc"), null);
  lika(tolkaTal(null), null);
  lika(tolkaTal(0), 0, "en medveten nolla är ett värde");
});

prov("betyg hålls inom skalan", () => {
  lika(tolkaBetyg("4,2"), 4.2);
  lika(tolkaBetyg("9"), 5, "utanför skalan är det inget betyg");
  lika(tolkaBetyg("-2"), 0);
  lika(tolkaBetyg(""), null);
});

prov("betyg och procent skrivs med svenskt komma", () => {
  lika(betygstext(3.7), "3,7");
  lika(betygstext(4), "4,0");
  lika(betygstext(null), "—");
  lika(procenttext(13.5), "13,5 %");
  lika(procenttext(14), "14 %", "hela tal får ingen påhängd nolla");
  lika(kronor(1299), "1 299", "hårt mellanrum, så att beloppet aldrig bryts");
});
...
prov("snedstreck delar inte druvor", () => {
  ...
  lika(tolkaLista("Shiraz/Syrah, Tempranillo"), ["Shiraz/Syrah", "Tempranillo"]);
  lika(tolkaLista("Chardonnay\nRiesling"), ["Chardonnay", "Riesling"]);
  lika(tolkaLista("  "), []);
  lika(tolkaLista(["Nebbiolo", "  ", ""]), ["Nebbiolo"]);
  lika(skrivLista(["a", "b"]), "a, b");
});
...
prov("bara http och https duger som adress", () => {
  ...
  lika(trygsamUrl("https://vivino.com/x"), "https://vivino.com/x");
  lika(trygsamUrl("javascript:alert(1)"), "");
  lika(trygsamUrl("vivino.com"), "");
  lika(trygsamUrl(null), "");
});

/* --- vinets namn -------------------------------------------------- */

prov("producenten utelämnas när namnet redan börjar med den", () => {
  lika(vinTitel(DATA.viner[0]), "Félix Solís Mucho Más Tinto N.V.");
  const eget = tolkaVinData({
    viner: [{ id: "x", namn: "Vietti Barolo", producent: "Vietti", argang: "2018" }],
  });
  lika(vinTitel(eget.viner[0]), "Vietti Barolo 2018", "ingen dubblering");
});

prov("ett namnlöst vin heter något ändå", () => {
  const d = tolkaVinData({ viner: [{ id: "x" }] });
  lika(vinTitel(d.viner[0]), "Namnlöst vin");
});

prov("underraden bär bara det som är ifyllt", () => {
  lika(vinUnderrad(DATA.viner[0]), [
    "Spanien · La Mancha",
    "Shiraz/Syrah, Tempranillo",
    "13,5 %",
  ]);
  lika(vinUnderrad(DATA.viner[3]), [], "ett tomt vin får ingen underrad");
});
...
prov("faktatabellen lämnar ingen rad efter tomma fält", () => {
  ...
  const rader = vinFakta(DATA.viner[0]);
...
prov("lagret och drickandet står bara där de betyder något", () => {
  ...
  // en gåta.
  const vill = vinFakta(DATA.viner[3]).map((r) => r.etikett);
  if (vill.includes("I källaren")) throw new Error("källarrad på ett vin man vill prova");
  if (vill.includes("Drucket")) throw new Error("dryckesdatum på ett oprovat vin");

  const drucket = vinFakta(DATA.viner[1]);
...
prov("en flaska heter flaska och två heter flaskor", () => {
  lika(vinFakta(DATA.viner[2]).find((r) => r.etikett === "I källaren")?.varde, "1 flaska");
});
...
prov("ett nyss tillagt vin är tomt", () => {
  ...
  const tomt = tolkaVinData({ viner: [{ id: "x" }] });
  lika(arTomt(tomt.viner[0]), true);
  // Ett enda ifyllt fält räcker för att det skall finnas något att läsa.
  lika(arTomt(DATA.viner[0]), false);
  lika(arTomt(DATA.viner[3]), false, "namnet ensamt räcker");
});

prov("smaknotens fot skrivs som på förlagan", () => {
  lika(
    smaknotsFot({ id: "x", ord: "Vanilj, ek, tobak", grupp: "fatad", antal: 1511 }),
    "1 511 kommentarer om fatad toner"
  );
  lika(
    smaknotsFot({ id: "x", ord: "a", grupp: "röd frukt", antal: 1 }),
    "1 kommentar om röd frukt toner",
    "en kommentar är inte kommentarer"
  );
  ...
  // av siffran.
  lika(smaknotsFot({ id: "x", ord: "a", grupp: "jordig", antal: null }), "jordig toner");
  lika(smaknotsFot({ id: "x", ord: "a", grupp: "", antal: null }), "");
});
...
prov("en halvskriven lista räknas som i takt", () => {
   ...
   */
  lika(tolkaLista("nötkött, "), ["nötkött"]);
  lika(tolkaLista("nötkött,"), ["nötkött"]);
  lika(tolkaLista("nötkött "), ["nötkött"]);
  lika(tolkaLista("nötkött, pasta, "), ["nötkött", "pasta"]);
  // Mellanslag INNE i en post är en del av posten och får aldrig strykas.
  lika(tolkaLista("torkad frukt, mörk choklad"), ["torkad frukt", "mörk choklad"]);
  lika(tolkaLista("Pinot Noir"), ["Pinot Noir"]);
});
...
prov("utskriften är den man skrev in", () => {
  ...
  for (const rad of ["nötkött, pasta", "Shiraz/Syrah, Tempranillo", "Pinot Noir"]) {
    lika(skrivLista(tolkaLista(rad)), rad);
  }
});
...
prov("läget är ett flöde som går runt", () => {
  lika(LAGEN.map((l) => l.id), ["vill", "har", "drucken"]);
  lika(nastaLage("vill"), "har");
  lika(nastaLage("har"), "drucken");
  lika(nastaLage("drucken"), "vill");
  lika(lagesIndex("drucken"), 2);
});

prov("okänt läge och okänd typ faller tillbaka", () => {
  const d = tolkaVinData({
...
prov("ett vin i källaren utan antal är en flaska", () => {
  ...
  lika(flaskor(DATA.viner[2]), 1);
  lika(flaskor(DATA.viner[0]), 3);
  lika(flaskor(DATA.viner[1]), 0, "ett drucket vin är ingen flaska");
  lika(flaskor(DATA.viner[3]), 0, "ett vin man vill prova är ingen flaska");
});
...
prov("räkneverket", () => {
  ...
  lika(Math.round(andelDrucken(r) * 100), 25);
});

prov("ett tomt register ger inga nollor att dela med", () => {
  const r = rakna(TOM_VIN_DATA);
  lika(r, { vill: 0, har: 0, drucken: 0, totalt: 0, flaskor: 0, varde: 0 });
  lika(andelDrucken(r), 0);
  lika(medelbetyg(TOM_VIN_DATA, "eget"), null);
});
...
prov("snittet räknas bara på dem som har ett betyg", () => {
  ...
  lika(medelbetyg(DATA, "eget"), 3.6, "(4 + 3,2) / 2");
  lika(medelbetyg(DATA, "vivino"), (3.7 + 4.1 + 4.4) / 3);
});

prov("oense räknas bara när båda betygen finns", () => {
  lika(Math.round(oense(DATA.viner[0])! * 10) / 10, 0.3);
  lika(Math.round(oense(DATA.viner[1])! * 10) / 10, -0.9);
  lika(oense(DATA.viner[2]), null, "utan eget betyg finns ingen skillnad");
});
...
prov("ett vin märkt som omöjligt att slå upp räknas inte som ofyllt", () => {
  ...
  lika(harProfil(DATA.viner[3].profil), false);
  lika(DATA.viner[3].uppgifterSaknas, true);
  lika(ofullstandiga(DATA).map((v) => v.kod), ["VIN-003"]);
});
...
prov("ofyllda tal hamnar sist åt båda hållen", () => {
  ...
  // den.
  lika(sorteraViner(DATA.viner, "pris").map((v) => v.kod), [
    "VIN-001",
    "VIN-002",
    "VIN-003",
    "VIN-004",
  ]);
  lika(sorteraViner(DATA.viner, "betyg").map((v) => v.kod), [
    "VIN-003",
    "VIN-001",
    "VIN-002",
    "VIN-004",
...
prov("kodordningen är den som inte flyttar sig", () => {
  lika(sorteraViner([...DATA.viner].reverse(), "kod").map((v) => v.kod), [
    "VIN-001",
    "VIN-002",
    "VIN-003",
    "VIN-004",
...
prov("sökningen träffar namn, land, druva och kod", () => {
  lika(filtreraViner(DATA, "tempranillo", null, null).map((v) => v.kod), [
    "VIN-001",
  ]);
  lika(filtreraViner(DATA, "italien", null, null).map((v) => v.kod), [
    "VIN-003",
  ]);
  lika(filtreraViner(DATA, "VIN-002", null, null).map((v) => v.kod), [
    "VIN-002",
  ]);
  lika(
    filtreraViner(DATA, "spanien röda", null, null).map((v) => v.kod),
    ["VIN-001"],
    "alla termer måste träffa"
  );
});

prov("läge och typ filtrerar samtidigt", () => {
  lika(filtreraViner(DATA, "", "har", null).map((v) => v.kod), [
    "VIN-001",
    "VIN-003",
  ]);
  lika(filtreraViner(DATA, "", "har", "rott").map((v) => v.kod), [
    "VIN-001",
    "VIN-003",
  ]);
  lika(filtreraViner(DATA, "", "har", "vitt"), []);
});
...
prov("ett vin med två druvor räknas i båda grupperna", () => {
  ...
  const d = fordelning(DATA.viner, "druva");
...
prov("viner utan värde samlas under Ej ifyllt", () => {
  ...
  const l = fordelning(DATA.viner, "land");
...
prov("typens färg är bunden och byts aldrig", () => {
  ...
  const d = fordelning(DATA.viner, "typ");
  lika(d.find((x) => x.namn === "Rött")?.ton, typTon("rott"));
  lika(d.find((x) => x.namn === "Vitt")?.ton, typTon("vitt"));
  lika(d.find((x) => x.namn === "Rosé")?.ton, typTon("rose"));
});

prov("fördelningen sorteras störst först", () => {
  lika(fordelning(DATA.viner, "typ")[0].namn, "Rött");
});
...
prov("betyg mot pris kräver båda talen", () => {
  ...
  const p = betygMotPris(DATA.viner, null);
...
prov("det öppnade vinet framhävs i diagrammet", () => {
  const p = betygMotPris(DATA.viner, "b");
  lika(p.map((x) => x.framhavd), [false, true, false]);
});

prov("smakkartan kräver båda skalorna", () => {
  const p = smakkarta(DATA.viner, "fyllighet", "stravhet", null);
  lika(p.map((x) => x.id), ["a", "b"]);
  lika([p[0].x, p[0].y], [70, 34]);
  lika(
    smakkarta(DATA.viner, "sotma", "syra", null).map((x) => x.id),
    ["a", "b"]
  );
});
...
prov("prisaxeln rundas upp och har ett golv", () => {
  ...
  lika(pristak(betygMotPris(DATA.viner, null)), 600);
  lika(pristak([]), 200);
  lika(
    pristak([{ id: "x", etikett: "", x: 45, y: 3, ton: 0, framhavd: false }]),
    200
  );
});
...
prov("en okänd smakgrupp får blyerts och ingen gissad färg", () => {
  ...
  lika(gruppTon("fatad"), 0);
  lika(gruppTon("Svart frukt"), 5, "versaler spelar ingen roll");
  lika(gruppTon("hittepå"), 3);
  lika(gruppTon(""), 3);
});
...
prov("koden är treställig och räknas aldrig ned", () => {
  lika(formateraKod(1), "VIN-001");
  lika(formateraKod(14), "VIN-014");
  lika(formateraKod(0), "VIN-001");
  // Räknaren kan ha hamnat efter när två enheter skrivit offline. Då
  // vinner det högsta använda numret.
  lika(nastaLedigaKod(DATA), 5);
  lika(nastaLedigaKod({ ...DATA, nastaKod: 1 }), 5);
  lika(nastaLedigaKod({ ...DATA, nastaKod: 99 }), 99);
  lika(nastaLedigaKod(TOM_VIN_DATA), 1);
});

/* --- tolkningen --------------------------------------------------- */

prov("trasig data kraschar inte", () => {
  lika(tolkaVinData(null), TOM_VIN_DATA);
  lika(tolkaVinData("strunt"), TOM_VIN_DATA);
  lika(tolkaVinData({ viner: "inte en lista" }).viner, []);
  lika(tolkaVinData({ viner: [null, 3, { id: "x" }] }).viner.length, 1);
});
...
prov("klam håller sig inom spannet", () => {
  lika(klam(120, 0, 100), 100);
  lika(klam(-4, 0, 100), 0);
  lika(klam(50, 0, 100), 50);
});
